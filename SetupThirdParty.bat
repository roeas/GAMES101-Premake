...
rem TODO : Remove some useless modules.
cmake -S %OPENCV_PATH% -B %OPENCV_PATH%/build -D OPENCV_DOWNLOAD_MIRROR_ID=gitcode -D BUILD_TESTS=OFF -D BUILD_PERF_TESTS=OFF -D BUILD_EXAMPLES=OFF -D BUILD_opencv_apps=OFF -D BUILD_opencv_world=ON
cmake -S . -B build
