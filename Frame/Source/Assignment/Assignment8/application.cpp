...
// Enable anti-aliasing and circular points.
    ...
    // Create two ropes 
    ropeEuler = new Rope(Vector2D(0, 200), Vector2D(-400, 200), 100, config.mass, config.ks, { 0 });
    ropeVerlet = new Rope(Vector2D(0, 200), Vector2D(-400, 200), 100, config.mass, config.ks, { 0 });
}
...
//Simulation loops
    ...
    {
        ropeEuler->simulateEuler(1.0f / config.steps_per_frame, config.gravity);
        ropeVerlet->simulateVerlet(1.0f / config.steps_per_frame, config.gravity);
    }
    // Rendering ropes
    Rope *rope;
